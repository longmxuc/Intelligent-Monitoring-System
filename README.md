...
- **双串口上报**：支持 Air780e（4G 模块）与蓝牙串口（如 HC-05 等），依据 BLE 状态自动选择输出通道。
- **远程控制**：通过文本指令 `ONMQ2` / `OFFMQ2` 控制 MQ2 供电，实现远程节能与调试。
...
| 蓝牙模块（HC-05 等） | USART3 + DMA | 本地 BLE/串口透传 |
...
| 蓝牙串口（HC-05 等） | `PB10` (`USART3_TX`) → 模块 RX | `PB11` (`USART3_RX`) ← 模块 TX | 9600 bps + DMA，`BLE_STATE` 决定是否发射 |

> 注意：所有外设应与 MCU 共地；若外设逻辑为 5 V（例如部分蓝牙模组），请使用电平转换或确认其兼容 3.3 V TTL。
...
- 添加了STM32状态栏功能，它可以向服务器发起请求，实时接收当前时间和在线人数（基于WebSocket判断）。
...
> ……其余 13 条请查看在线网站`https://znhj.iepose.cn`-功能-关于项目 或者 `web/changelog-data.js`
<!-- CHANGELOG_END -->
